...
# Currently only support T-Mobile 

file=$1
output=$file
# Generate all the necessary tmp file
../Tools/plot/detail_trace_generator.py < $file
#tit=$2
point_size=1.8
gnuplot -p << EOF
...
set style line 4 lt 1 ps $point_size lc rgb "red" lw 2
# customize label
set ytics("PCH" 1, "PCH/FACH Transition" 2, "FACH" 3, "FACH/DCH Transition" 4, "DCH" 5)
plot "tmp_IP.txt" using 1:2 with points pt 7 ps $point_size lc rgb "red" title "IP packets"
...
set style line 2 lt 9 pt 8 ps $point_size lc rgb "orange" lw 2
...
#"tmp_FACH_to_PCH_start.txt" using 1:2with points pt 5 lc rgb 'sea-green' title "FACH_to_PCH Begin", \
...
#      "tmp_RLC_end.txt" using 1:2 with points pt 10 lc rgb 'purple' title "RLC Mapped Last PDU"

plot  "tmp_FACH_to_DCH_start.txt" using 1:2 with points pt 1 ps $point_size lc rgb 'blue' title "FACH_to_DCH Begin", \
      "tmp_FACH_to_DCH_end.txt" using 1:2 with points pt 2 ps $point_size lc rgb 'red' title "FACH_to_DCH End", \
      "tmp_DCH_to_FACH_start.txt" using 1:2 with points pt 3 ps $point_size lc rgb 'green' title "DCH_to_FACH Begin", \
      "tmp_DCH_to_FACH_end.txt" using 1:2 with points pt 4 ps $point_size lc rgb 'cyan' title "DCH_to_FACH End", \
      "tmp_RLC.txt" using 1:2 with linespoints linestyle 2 title "Mapped RLC PDUs"
